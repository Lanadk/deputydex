# 📚 Clean Architecture dans une app Next.js
...
Contient :
- entités internes (`entities/`, proches de la row DB — ce que les repositories retournent)
- DTO exposés au front (`dto/`)
- mappers entité → DTO (`mappers/`, la seule couche où se fait cette conversion)
- contrats de repository (`repositories/IXRepository.ts`)
- contrats de gateway (`gateways/`)
- use cases
...
### 5. Repository
Le repository concret connaît Prisma. Il implémente le contrat `IActeursRepository` défini dans le domaine.
...
- retourner des `ActeurEntity` (la row Prisma typée comme entité du domaine)

---

### 6. Mapper
Le mapper convertit l’entité en DTO exposé. Il vit dans le domaine, pas dans l’infra — c’est la seule couche où se fait cette conversion.

Exemple :

```txt
app/domains/acteurs/mappers/acteur.mapper.ts
```

Flux :

```txt
Prisma row (typée ActeurEntity) -> mapper (domaine) -> ActeurDTO
```
...
```txt
(ui) -> domains = L'UI consomme ce que le métier expose.
(ui) -> _shared =  L'UI utilise les types et helpers communs.
(ui) -> (ui)/gateways = L'UI délègue ses appels HTTP à ses gateways.
api -> domains/use-cases =  La route orchestre, le use case décide.
api -> infrastructure uniquement pour injecter les impl = La route branche les implémentations concrètes, c'est son seul contact avec l'infra.
infrastructure -> domains =  L'infra implémente les contrats du domaine.
infrastructure -> _shared = L'infra utilise les types et helpers partagé.
domains -> _shared = Le domaine s'appuie sur le shared pour les types transverses, la pagination, etc.
```

### Interdit

```txt
domains -> (ui) = Le métier ne sait pas qu'il y a un navigateur.
domains -> prisma directement = Le domaine définit des contrats, il ne requête pas.
(ui) -> prisma = La base de données n'existe pas côté navigateur.
(ui) -> use-cases serveur directement = Tout passe par HTTP, jamais en direct.
_shared -> React =  Le shared est neutre, il ne dépend d'aucun framework.
_shared -> Prisma =  Le shared est neutre, il ne dépend d'aucune DB.
```
...
5. `_shared` ne contient que du neutre.
