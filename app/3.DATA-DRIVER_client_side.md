# 📚 Architecture Data-Driven : Component Library
...
```tsx
// template/page-content/page-content-lib.tsx
...
### 2. Composants réutilisables (headers / sections)

```tsx
// template/headers/page-header/page-header-lib.tsx
export const PageHeaderLib = ({ title, subtitle }) => (
  <div className="mb-8">
    <h2>{title}</h2>
...
// template/sections/code-block-section/component-section-code-block-lib.tsx
export const ComponentSectionCodeBlockLib = ({ title, code, children }) => (
  <div>
    <h3>{title}</h3>
    <div className="mt-4">
      <CodeBlock code={code}>
...
> Convention du projet : les composants de la component library portent le suffixe `Lib` (`PageHeaderLib`, `ComponentSectionCodeBlockLib`, `ButtonLib`...).

### 3. Constants (configuration des sections)

```tsx
// atoms/button/button-lib.constants.tsx
...
5. **Un fichier par composant** : `button-lib.constants.tsx`, `badge-lib.constants.tsx`, etc.
